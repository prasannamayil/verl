...
"""
Preprocess the CuratedThoughts dataset to parquet format
"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--local_dir', default='~/data/curated_thoughts')
    ...
    data_source = 'bethgelab/CuratedThoughts'
    print(f"Loading the {data_source} dataset from huggingface...", flush=True)
    dataset = datasets.load_dataset(data_source, "OpenR1-Math-220k-default")
    ...
    def make_map_fn(split):
        ...
        def process_fn(example, idx):
            ...
            answer = example.pop('answer', None)
            question_type = example.pop('question_type', None)
            source = example.pop('source', None)
            uuid = example.pop('uuid', None)
